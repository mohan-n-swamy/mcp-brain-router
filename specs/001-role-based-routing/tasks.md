...
- [x] **T1 — Provider + Role enums + id→provider table** (`router.py`)
...
- [x] **T2 — `[roles]` config parsing** (`config.py`, `config.toml`)
...
- [x] **T3 — `resolve_role()` + `Assignment` dataclass** (`router.py`) — CORE
...
- [x] **T4 — delegate() gains `role` + `orchestrator`** (`server.py`)
